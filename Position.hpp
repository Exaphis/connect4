...
class Position {
 public:
    ...
    // prints a given position
    void print() const {
        std::cout << "Moves: " << moves << '\n';
        static constexpr int col_start[] = { 0, 7, 14, 21, 28, 35, 42 };

        char curr_player = moves % 2 == 0 ? 'X' : 'O';
        char other_player = moves % 2 == 0 ? 'O' : 'X';

        for (int r = HEIGHT - 1; r >= 0; r--) {
            for (int c = 0; c < WIDTH; c++) {
                position_t cell_mask = UINT64_C(1) << (r + col_start[c]);

                std::cout << '|';
                if (current_position & cell_mask) {
                    std::cout << curr_player;
                }
                else if (mask & cell_mask) {
                    std::cout << other_player;
                }
                else {
                    std::cout << ' ';
                }
    ...
    char get_curr_char() const {
        return moves % 2 == 0 ? 'X' : 'O';
    }

    char get_opp_char() const {
 ...
 private:
    ...
    static bool alignment(position_t pos) {
        // horizontal 
        position_t m = pos & (pos >> (HEIGHT+1));
